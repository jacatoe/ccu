/*
    ...
    CSCI 473
    MPI06
    mc-integration-parallel.c - Integrates a function using the Monte Carlo method (parallel version).
*/
...
int main(int argc, char* argv[]) {
    ...
    int a;        // Left function bound
    int b;        // Right function bound
    ...
    if(argv[1]!=NULL) {a=(int)strtol(argv[1], &end_a, 10);}
    if(argv[2]!=NULL) {b=(int)strtol(argv[2], &end_b, 10);}
    ...
    if(argc!=4) {printf("Usage: ./mc-integration-serial [LEFT BOUND] [RIGHT BOUND] [POINTS]\n");}
        ...
        if(my_rank==0) {printf("%i  %i  %i    %f    %f    %i\n", a, b, n, area, elapsed_time, size);}
